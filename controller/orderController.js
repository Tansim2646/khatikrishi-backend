...
const getAllOrders = async (req, res) => {
  ...
  if (!status) {
    queryObject.status = {
      $in: ["Pending", "Processing", "Delivered", "Cancel"].map(
        (status) => new RegExp(status, "i")
      ),
    };
    console.log("status", status);
  } else {
    queryObject.status = new RegExp(status, "i");
  }

  if (customerName) {
    queryObject.$or = [
      { "user_info.name": new RegExp(customerName, "i") },
      { invoice: new RegExp(customerName, "i") },
    ];
  }
